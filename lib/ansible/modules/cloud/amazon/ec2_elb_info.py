...
class ElbInformation(object):
    ...
    def _get_tags(self, elbname):
        ...
        elb_tags = self.connection.get_list(
            'DescribeTags', params, [('member', Tag)])
        return dict((tag.Key, tag.Value)
                    for tag in elb_tags if hasattr(tag, 'Key'))
...
def main():
    ...
    argument_spec.update(dict(
        ...
        use_boto3={'default': False, 'type': 'bool'}
    )
    )
